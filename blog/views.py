        ...
        if uf.is_valid():
            ...
            if username != None and password != None and email != None:
                try:
                    ...
                    userlogin = auth.authenticate(username=username,
                                                  password=password)
...
def ajax_getcontents(request):
    if request.method == "POST":
        ...
        if current == -1 or num == -1:
            ...
            return Http404(ret)
        posts = ContentItems.objects.all().order_by(
            '-posttime')[current:current + num]
...
def own_zone(request):
    ...
    return render(request, 'ownzone.html', {
        'username': username,
        'posts': posts
    })
...
def ajax_submit(request):
    ...
    if request.method == "POST":
        ...
        contentitem = ContentItems.objects.create(uid=request.user,
                                                  postposition='',
                                                  content=content,
                                                  isshowname=True)
        contentitem.save()
    ...
    if request.method == "POST":
        ...
        if cmt != "" and cmt != None:
            ...
            if cid != -1:
                ...
                if content:
                    newcomment = Comment.objects.create(uid=request.user,
                                                        cid=content,
                                                        cuid=content.uid,
                                                        comment=cmt)
                    newcomment.save()
